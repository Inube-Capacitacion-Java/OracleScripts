-- Un trigger en Oracle SQL es un bloque de código (escrito en PL/SQL) que se ejecuta automáticamente en respuesta a un
-- evento específico que ocurre en una tabla, como una operación de inserción (INSERT), actualización (UPDATE) o
-- eliminación (DELETE). Su función principal es automatizar tareas, hacer cumplir reglas de negocio, validar datos,
-- auditar cambios o mantener la integridad referencial sin necesidad de intervención manual
...
-- Crear la Bitácora de la tabla teléfonos
...
-- Crear la Bitácora de la tabla Facturas
...
BEGIN
  ...
  ELSIF UPDATING THEN
    ...
    V_FECHA_MODIFICACION := SYSDATE;
  END IF;
  INSERT INTO FACTURAS_BIT (ID_FACTURA_BIT, ID_FACTURA, ID_CLIENTE, MONTO_TOTAL, FOLIO, ANIO,
  FECHA_FACTURA, FECHA_CREACION, FECHA_MODIFICACION, ACCION, IP)
    VALUES (
...
-- Crear la Bitácora de la tabla Pagos
...
BEGIN
  ...
  ELSIF UPDATING THEN
    ...
    V_FECHA_MODIFICACION := SYSDATE;
  END IF;
  INSERT INTO PAGOS_BIT (ID_PAGO_BIT, ID_PAGO, ID_FACTURA, MONTO, FECHA_PAGO,
